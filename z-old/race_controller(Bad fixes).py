...
#         for i in ports:        
            print ('lane %s :: ports %i :: %s' % (LANES[i], i, ready_or_not(i)))
...
def reset_race(port):
    global go
    go = 0
    #Hopfully exit a failed race
    print ("Resetting Race")
    sleep(.5)
    return
    
## when a rising edge is detected on a lane port the starter callback will be run
GPIO.add_event_detect(RESET, GPIO.FALLING, callback=reset_race, bouncetime=200)
...
def main_pgm():
                    ...
                    if len(SHOWN_TIMES) < len(HEAT_LANES):   
                        ...
                        for port in LANE_TIMES:
                            if LANE_TIMES[port] and port in HEAT_LANE_PORTS and port not in SHOWN_TIMES:
                                #some function here
                                _thread.start_new_thread( fnHeatPlace, (port,))
    #                             d = LANE_TIMES[port] - START_TIME
    #                             heat = Heat.query.filter_by(id=HEAT, category=CATEGORY, lane=LANES[port]).first()
    #                             heat.time = float("%s.%s" % (d.seconds, d.microseconds))
    #                             app.db.session.add(heat)
    ...
    # #                     pit_update(CATEGORY.id, HEAT)
    #                     UpdateScoreboard().start()
                if (go != '' and len(SHOWN_TIMES) >= len(HEAT_LANES)) :
    ...
    #                 HEAT_LANE_PORTS = lambda x: LANE_GPIO_PORTS[x.lane-1], HEAT_LANES
                    ...
                    print ("")
                    gate_settle = 3 #time to wait for startgate to be reset
                    while 1:
