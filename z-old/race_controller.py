...
def fnPrintOBJs():
    try:
        sys.stdout.write('\r                          ')
        print("Heat Lane Ports")
        print (list(HEAT_LANE_PORTS))
        sys.stdout.write('\r                          ')
        print ("Lane Times")
        print (list(LANE_TIMES))
        sys.stdout.write('\r                          ')
        print ("shown Times")
        print (list(SHOWN_TIMES))     
    except:
        pass    
    ...
    try:
        ...
        if START_TIME:
            if len(SHOWN_TIMES) >= len(HEAT_LANES):
                ...
                sys.stdout.flush()
#                 fnPrintOBJs()
#                 LANE_TIMES = list(dict(zip(LANE_GPIO_PORTS, map(lambda x: LANE_GPIO_PORTS[x.lane-1], HEAT_LANES))))
#                 for port in LANE_TIMES:
#                 while len(SHOWN_TIMES) < len(HEAT_LANES):
#                     d = datetime.now() - START_TIME
#                     sys.stdout.write('\r%i.%i' % (d.seconds, d.microseconds))
#                     sys.stdout.flush()                    
#                     #changed to while below was a for statement
                for port in LANE_TIMES:
#                     x = port
#                     print (x)
